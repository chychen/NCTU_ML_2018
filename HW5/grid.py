...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
            # NOTE: modified by me
			self.svmtrain_pathname = os.path.join(dirname, '/usr/local/Cellar/libsvm/3.22/bin/svm-train')
			self.gnuplot_pathname = '/usr/local/Cellar/gnuplot/5.2.3/bin/gnuplot'
		else:
			# example for windows
